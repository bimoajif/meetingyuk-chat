...
class AuthController extends GetxController {
  ...
  Future<void> getUser() async {
    try {
      final response = await dio
          .get("${dotenv.get('AUTH_ENDPOINT_URL', fallback: "URL NOT FOUND")}/user");
  ...
  void login(
    ...
    currentUser.value.isMerchant == 1
        ? box.write(
            'priv_key',
            dotenv.get('MERCHANT_PRIVKEY', fallback: "0"),
          )
        : box.write(
            'priv_key',
            dotenv.get('USER_PRIVKEY', fallback: "0"),
          );
  ...
  Future<String?> fetchCityName(latitude, longitude) async {
    return await getCityFromCoordinates(latitude, longitude);
  }
}
